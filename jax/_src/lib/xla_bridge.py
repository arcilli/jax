...
def _get_pjrt_plugin_names_and_library_paths(
  ...
  for plugin in plugins_from_env.split(','):
    try:
      ...
      logger.warning(
          'invalid value %s in env PJRT_NAMES_AND_LIBRARY_PATHS %s',
          plugin,
          plugins_from_env,
      )
  return pjrt_plugins
...
def register_pjrt_plugin_factories(plugins_from_env: str):
  """Registers a backend factory for every plugin found."""
...
if lib.xla_extension_version >= 125:
  # The plugin names and paths are set in ENV PJRT_NAMES_AND_LIBRARY_PATHS, in
  # the format of 'name1:path1,name2:path2' ('name1;path1,name2;path2' for
  # windows).
